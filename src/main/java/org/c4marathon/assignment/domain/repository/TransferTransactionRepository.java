...
public interface TransferTransactionRepository extends JpaRepository<TransferTransaction, Long> {
	...
	@Query("SELECT t FROM TransferTransaction t WHERE t.status = 'PENDING' AND t.expiredAt <= :remindTime")
	List<TransferTransaction> findRemindTargetTransactions(LocalDateTime remindTime);
}
